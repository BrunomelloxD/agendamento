...
export class UserRepository implements IUserRepository {
  ...
  async findAll({
    ...
    limit = 10,
    search,
  }: PaginationDto): Promise<PaginatedResponseDto<UserResponseDto>> {
    const [users, total] = await this.prismaService.$transaction([
      ...
      this.prismaService.user.count({ where: { deleted_at: null } }),
    ]);
  ...
  async existsById(id: string, deleted_at_filter?: boolean): Promise<boolean> {
    ...
    return (
      (await this.prismaService.user.count({ where: { id, ...deleted_at } })) >
      0
    );
  }
}
