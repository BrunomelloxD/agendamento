...
describe('UserService', () => {
  ...
  describe('existById', () => {
    ...
    it('should return false when user does not exist with id', async () => {
      ...
      expect(mockUserRepository.existsById).toHaveBeenCalledWith(
        '999',
        undefined,
      );
      expect(result).toBe(false);
    ...
    it('should throw ConflictException when email already exists', async () => {
      ...
      await expect(service.create(mockCreateData)).rejects.toThrow(
        new ConflictException(
          'User with email bruno@example.com already exists.',
        ),
      );
  ...
  describe('remove', () => {
    ...
    it('should throw NotFoundException when user does not exist', async () => {
      ...
      await expect(service.remove('999')).rejects.toThrow(
        new NotFoundException(
          'User with id 999 does not exists or is already deleted.',
        ),
      );
    ...
    it('should throw NotFoundException when user is already deleted', async () => {
      ...
      await expect(service.remove('1')).rejects.toThrow(
        new NotFoundException(
          'User with id 1 does not exists or is already deleted.',
        ),
      );
