...
describe('PasswordRecoveryRepository', () => {
  ...
  beforeEach(async () => {
    ...
    repository = module.get<PasswordRecoveryRepository>(
      PasswordRecoveryRepository,
    );
  ...
  describe('recoverPassword', () => {
    it('should create a password reset code successfully', async () => {
      ...
      expect(mockPrismaService.passwordResetCode.create).toHaveBeenCalledTimes(
        1,
      );
    ...
    it('should throw error when create fails', async () => {
      ...
      await expect(
        repository.recoverPassword(code, expiresAt, userId),
  ...
  describe('findByRecoveryCode', () => {
    it('should return true when valid recovery code exists', async () => {
      ...
      expect(
        mockPrismaService.passwordResetCode.findFirst,
    ...
    it('should throw error when findFirst fails', async () => {
      ...
      await expect(repository.findByRecoveryCode(code, userId)).rejects.toThrow(
        'Database error',
      );
  ...
  describe('delete', () => {
    it('should mark unused password reset codes as used successfully', async () => {
      ...
      mockPrismaService.passwordResetCode.updateMany.mockResolvedValue({
        count: 2,
      });
      ...
      expect(
        mockPrismaService.passwordResetCode.updateMany,
      ).toHaveBeenCalledTimes(1);
      expect(
        mockPrismaService.passwordResetCode.updateMany,
      ).toHaveBeenCalledWith({
    ...
    it('should throw error when updateMany fails', async () => {
      ...
      await expect(repository.delete(userId)).rejects.toThrow('Database error');
  ...
  describe('Integration scenarios', () => {
    it('should handle complete password recovery flow', async () => {
      ...
      const _found = await repository.findByRecoveryCode(code, userId);
      expect(
        mockPrismaService.passwordResetCode.findFirst,
      ).toHaveBeenCalledTimes(1);
