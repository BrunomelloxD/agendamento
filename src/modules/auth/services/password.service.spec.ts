...
describe('PasswordService', () => {
  ...
  describe('compare', () => {
    ...
    it('should throw error when compare fails', async () => {
      ...
      await expect(
        service.compare(plainPassword, hashedPassword),
  ...
  describe('recoverPassword', () => {
    it('should initiate password recovery successfully', async () => {
      ...
      expect(passwordRecoveryRepository.delete).toHaveBeenCalledWith(
        mockUser.id,
      );
      ...
      expect(passwordRecoveryRepository.recoverPassword).toHaveBeenCalledTimes(
        1,
      );

      expect(mailService.sendRecoveryCode).toHaveBeenCalledWith(
        mockUser.email,
        '550000',
      );
    ...
    it('should generate a 6-digit recovery code', async () => {
      ...
      const recoverPasswordCall =
        passwordRecoveryRepository.recoverPassword.mock.calls[0];
  ...
  describe('verifyRecoveryCode', () => {
    ...
    it('should verify recovery code successfully', async () => {
      ...
      expect(
        passwordRecoveryRepository.findByRecoveryCode,
      ).toHaveBeenCalledWith(verifyData.code, mockUser.id);
    });
    ...
    it('should throw NotFoundException when recovery code returns null', async () => {
      userService.findByEmail.mockResolvedValue(mockUser);
      passwordRecoveryRepository.findByRecoveryCode.mockResolvedValue(
        null as any,
      );
  ...
  describe('Integration scenarios', () => {
    it('should complete full password recovery flow', async () => {
      ...
      expect(
        passwordRecoveryRepository.findByRecoveryCode,
      ).toHaveBeenCalledWith(code, mockUser.id);
