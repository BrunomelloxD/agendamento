...
describe('AuthController', () => {
  ...
  describe('signIn', () => {
    ...
    it('should throw NotFoundException when user does not exist', async () => {
      const userNotFoundError = new NotFoundException(
        `User with email ${loginDto.email} does not exists.`,
      );
    ...
    it('should throw UnauthorizedException when email is not verified', async () => {
      const emailNotVerifiedError = new UnauthorizedException(
        `Email not verified for user ${loginDto.email}.`,
      );
  ...
  describe('recoverPassword', () => {
    ...
    it('should request password recovery successfully', async () => {
      ...
      expect(passwordService.recoverPassword).toHaveBeenCalledWith(
        recoveryDto.email,
      );
    ...
    it('should throw NotFoundException when user email does not exist', async () => {
      ...
      await expect(controller.recoverPassword(recoveryDto)).rejects.toThrow(
        userNotFoundError,
      );
    ...
    it('should extract email from DTO correctly', async () => {
      ...
      expect(passwordService.recoverPassword).toHaveBeenCalledWith(
        differentRecoveryDto.email,
      );
      expect(passwordService.recoverPassword).not.toHaveBeenCalledWith(
        recoveryDto.email,
      );
    });
  });
  ...
  describe('verifyRecoveryCode', () => {
    ...
    it('should verify recovery code successfully', async () => {
      ...
      expect(result).toBeUndefined();
      expect(passwordService.verifyRecoveryCode).toHaveBeenCalledWith(
        verifyCodeDto,
      );
    ...
    it('should throw NotFoundException when user does not exist', async () => {
      const userNotFoundError = new NotFoundException(
        `User with email ${verifyCodeDto.email} not found`,
      );
      passwordService.verifyRecoveryCode.mockRejectedValue(userNotFoundError);

      await expect(
        controller.verifyRecoveryCode(verifyCodeDto),
    ...
    it('should throw NotFoundException when recovery code is invalid', async () => {
      const invalidCodeError = new NotFoundException(
        `Recovery code ${verifyCodeDto.code} not found`,
      );
    ...
    it('should pass complete DTO to service', async () => {
      ...
      expect(passwordService.verifyRecoveryCode).not.toHaveBeenCalledWith(
        verifyCodeDto,
      );
    });
  ...
  describe('resetPassword', () => {
    ...
    it('should throw NotFoundException when user does not exist', async () => {
      const userNotFoundError = new NotFoundException(
        `User with email ${resetPasswordDto.email} not found`,
      );
    ...
    it('should pass complete DTO to service', async () => {
      ...
      expect(passwordService.resetPassword).not.toHaveBeenCalledWith(
        resetPasswordDto,
      );
    });
  ...
  describe('Error handling consistency', () => {
    it('should maintain consistent error responses across all endpoints', async () => {
      ...
      const verifyDto: VerifyRecoveryCodeDto = {
        email: 'test@example.com',
        code: '123456',
      };
      ...
      await expect(controller.verifyRecoveryCode(verifyDto)).rejects.toThrow(
        notFoundError,
      );
